...
def restorePortalDatabase():
    ...
    if len(backupsFound) != len(portalSiteBackupsToBeRestored):
        print ("Time when error occurred :", time.asctime())
        sys.exit("ERROR : Some backups saved in the configuration NOT found in the list of portal backups in target APIC system.  Backups found : {}".format(backupsFound))
    else:
    ...
    while flag:
        ...
        if len(currentRunningList) == 0:
            ...
            print ("Waiting for already running portal site restores to be completed. Current time : ", time.asctime())
            time.sleep(120)
    ...
    if "backupInfo" in portalSubsystemDetails:
        ...
        for eachBackupId in portalSubsystemDetails["backupInfo"]:
            ...
            print (str(j) + ". Restoring portal backup : {} site url : {}  using restore CR name : {} ".format(eachBackupId, fileNameAndURL[fileNameAndURL.index("|")+1:], restoreCRName))
