                            ...
                            if ingressCACert != None and ingressCACert != "" and ingressCACert.lower().startswith("error") == False:
                                ...
                                if isAppliance:
                                    ...
                                    if output != None or output != "" or output.startswith("APIConnect") == True:
                                        ...
                                        config["management_subsystem"][mgmtSubsystemName]["root_ca_secret"] = "root-ca_cert.yaml"
...
def createPortalBackup(namespace):
    if portalSubsystemName != None:
        ...
        out = runKubernetesCommand("get portalbackups | grep Running | grep create| grep all", namespace, False, 10)
        lines = out.splitlines()
        runningBackups = {}
        for x in lines:
            runningBackups[x[0:x.index(" ")]] = "INPROGRESS"
        ...
        while flag:
            ...
            print ("Waiting for portal database backup to be completed. Current time : ", time.asctime())
            time.sleep(60)
            flag = False
            for eachRunningBackup in runningBackups:
                if runningBackups[eachRunningBackup] == "INPROGRESS":
