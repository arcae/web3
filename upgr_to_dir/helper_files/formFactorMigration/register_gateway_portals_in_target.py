                ...
                if new_endpoint != None and len(new_endpoint) > 0 and new_endpoint != "https://NEW_GATEWAY_ENDPOINT_HOST_IN_TARGET_SYSTEM":
                    ...
                    if oldGW != None and len(oldGW) > 0:
                        ...
                        if os.path.exists(oldGW[1]):
                            ...
                            del oldGWRecord["type"]
                            del oldGWRecord["api_version"]
                            del oldGWRecord["id"]
                            if oldGWRecord["oauth_shared_secret"].startswith("***"):
                                del oldGWRecord["oauth_shared_secret"]
                            del oldGWRecord["owned"]
                            del oldGWRecord["configuration"]
                            del oldGWRecord["state"]
                            del oldGWRecord["webhook_url"]
                            del oldGWRecord["availability_zone_url"]
                            ...
                            del oldGWRecord["created_at"]
                            del oldGWRecord["updated_at"]
                            del oldGWRecord["url"]
                            del oldGWRecord["org_url"]
                            ...
                            if oldGWRecord["endpoint"] not in gwData:
                                ...
                                created = 2
                                newGatewayId =  gwData[oldGWRecord["endpoint"]].strip() # name
                                print("")
                ...
                if new_endpoint != None and len(new_endpoint) > 0 and new_endpoint != "https://NEW_PORTAL_ENDPOINT_HOST_IN_TARGET_SYSTEM":
                    ...
                    if oldPortal != None and len(oldPortal) > 0:
                        ...
                        if os.path.exists(oldPortal[1]):
                            ...
                            del oldPortalRecord["type"]
                            del oldPortalRecord["api_version"]
                            del oldPortalRecord["id"]
                            del oldPortalRecord["owned"]
                            del oldPortalRecord["webhook_url"]
                            del oldPortalRecord["state"]
                            del oldPortalRecord["availability_zone_url"]
                            if "service_version" in oldPortalRecord:
                                del oldPortalRecord["service_version"]
                            del oldPortalRecord["created_at"]
                            del oldPortalRecord["updated_at"]
                            del oldPortalRecord["url"]
                            del oldPortalRecord["org_url"]
...
def updateData_OCPAndCP4i():
    ...
    if out1 != None and out1 != "" and out1.lower().startswith("error") == False:
        ...
        out3 = runKubernetesCommand("get secret " + topCRName + "-mgmt-admin-pass --no-headers", args.mgmt_ns, True, 1, False)
        if out3 != None and out3 != "" and out3.lower().startswith("error") == False:
            ...
            print("ACTION OUTPUT : Updated secret(" + topCRName + "-mgmt-admin-pass) with cloud manager password after restoring database. Configurator job will use this password in case of cp4i")
        
        print()
        #delete topCRName-configurator secret
        out2 = runKubernetesCommand("get secret " + topCRName + "-configurator --no-headers", args.mgmt_ns, True, 0, False)
        if out2 != None and out2 != "" and out2.lower().startswith("error") == False:
            runKubernetesCommand("delete secret " + topCRName + "-configurator", args.mgmt_ns, False)
            print("ACTION OUTPUT : Deleted secret(" + topCRName + "-configurator) in CP4I. When configurator job is rerun, it gets created again.")

        if "cp4i_registration_secret" not in config["management_subsystem"][selectedSubsystem]:
            print()
            #delete topCRName-cp4i-creds secret
            out3 = runKubernetesCommand("get secret " + topCRName + "-cp4i-creds --no-headers", args.mgmt_ns, True, 0, False)
            if out3 != None and out3 != "" and out3.lower().startswith("error") == False:
                runKubernetesCommand("delete secret " + topCRName + "-cp4i-creds", args.mgmt_ns, False)
                print("ACTION OUTPUT : Deleted secret(" + topCRName + "-cp4i-creds) in CP4I. When configurator job is rerun, it gets created again.")
        else:
