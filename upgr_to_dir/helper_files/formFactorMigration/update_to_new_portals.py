    ...
    if selectedSubsystem == None:
        ...
        for key in portalsFromSouceSystem:
            ...
            if "NEW_ENDPOINT" in eachPortal and "NEW_URL" in eachPortal:
                print("NEW PORTAL INSTANCE IS CREATED USING SCRIPT. New Portal NAME : {} , ENDPOINT : {}, WEB_ENDPOINT : {}".format(eachPortal["NEW_NAME"], eachPortal["NEW_ENDPOINT"], eachPortal["NEW_WEB_ENDPOINT_BASE"])) 
                ...
                print ("ACTION OUTPUT : NEW PORTAL INSTANCE IS 'NOT' CREATED USING THE SCRIPT for this source system portal")
        print()
        print()
                    ...
                    if portal_id != None:
                        ...
                        if alreadyUpdated == False:
                            ...
                            if mappingFoundFromOldToNew:
                                ...
                                runCommand("apic catalog-settings:update --org " + eachPORG["name"] + " --catalog "+ eachCatalog["name"] + " --server " + providerOrgServer + " " + f3, False)
                                ...
                                print ("ACTION OUTPUT : No mapping for this portal endpoint : {}. Leaving the portal as it is in the catalog : {} and org : {}".format(catalogSettingPortalEndpoint, eachCatalog["title"], eachPORG["title"]))
                    else:
        ...
        for key in portalServicesInTargetDB:
            ...
            if key in portalsFromSouceSystem:
                ...
                runCommand("apic portal-services:delete --server " + cloudAdminServer + " --org admin --availability-zone " + azoneid + " " + id1, False)
        ...
        while flag:
            ...
            if notReadyStateFound == False:
                ...
                print()
                print ("Waiting for portal site(s) creation, check every 2 minutes. current time : ", time.asctime())
        ...
        for eachPortalEndpoint in registeredPortalsInTarget:
            ...
            if eachPortalWebEndpoint in portalEndpointAndWebEPMap.values():
                allbackupInfo = runCommand("apic --mode portaladmin backups:list --server " + cloudAdminServer + " --portal_service_name " + pdata[3], True, 10, False)
                if "No remote backups found" not in allbackupInfo:
                    allbackupInfo = yaml.safe_load(allbackupInfo)
                    allBackupFileNamesList = allbackupInfo["backups"]
                    ...
                    for eachSubsysName in config_portal["portal_subsystem"].keys():
                        if "backupId_sites" in config_portal["portal_subsystem"][eachSubsysName]:
                            for eachBackupIdSaved in config_portal["portal_subsystem"][eachSubsysName]["backupId_sites"]:
                                ...
                                for eachBackupFileName in allBackupFileNamesList:
                                    if eachBackupIdSaved in eachBackupFileName:
                                        neededBackupFilename = eachBackupFileName
                                        orgCatalogInFilename = False
                                        for eachOrgCat in portalBackupInfo:
                                            if eachOrgCat in neededBackupFilename:
