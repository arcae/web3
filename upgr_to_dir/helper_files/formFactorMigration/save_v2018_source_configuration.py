    ...
    "template": {
      ...
      "spec": {
        ...
        "containers": [
          {
            "name": extract_job_name,
            "image": "apic-dev-docker-local.artifactory.swg-devops.com/ibm-apiconnect-management-v10-upgrade@sha256:7e32b7edbb218a5a616d323432abeed7ed824833fab0334078132954eea96b9d",
...
deploy_name = "v2018-mgmt-extract-download"
deploy_yaml = {
  "apiVersion": "apps/v1",
  "kind": "Deployment",
  "metadata": {
...
def getSubsystemDetails():
    ...
    if subsystemsList == None and subsystemsList == "":
        print ("ACTION OUTPUT : No API Connect subsystems are present in the project. Check the project and try gagain.")
        sys.exit(1)
    ...
    for eachSubsystem in subsystemsLinesArray:
        ...
        if subsystemType == "Management subsystem":
            ...
            for each in apicupyamlSubsystems:
                if each["kind"] == "apic.ibm.com/ManagementSubsystem":
        ...
        if subsystemType == "Portal subsystem":
            ...
            for each in apicupyamlSubsystems:
                if each["kind"] == "apic.ibm.com/PortalSubsystem":
            ...
            if args.skip_ptl_backup == False:
                ...
                if "site-backup-protocol" not in apicupyamlPTLSubsys["spec"]["settings"] or "site-backup-host" not in apicupyamlPTLSubsys["spec"]["settings"] or "site-backup-path" not in apicupyamlPTLSubsys["spec"]["settings"] or "site-backup-auth-user" not in apicupyamlPTLSubsys["spec"]["settings"] or "site-backup-auth-pass" not in apicupyamlPTLSubsys["spec"]["settings"]:
                    print ("Time when error occurred :", time.asctime())
                    sys.exit("ERROR : Portal subsystem ({}) validation failed. Database backup not configured.  Exiting...".format( ))
...
def createManagementBackup():
    ...
    if downloadCSVDeploy != None and downloadCSVDeploy.lower().startswith(deploy_name):
        ...
        print ("ACTION OUTPUT : " + deploy_name + " used to download csv not found. Will create it.")
    ...
    print ("ACTION OUTPUT : Extracted data from management subsystem")
    ...
    if args.skip_csv_download == False:
        ...
        if not os.path.exists(DATA_DIR + "/" + mgmtSubsystemName + "/extracted_data"):
            os.makedirs(DATA_DIR + "/" + mgmtSubsystemName + "/extracted_data", mode=0o755, exist_ok=True)

        runKubernetesCommand("cp " + args.mgmt_ns + "/" + neededPod + ":/upgrade " + DATA_DIR + "/" + mgmtSubsystemName + "/extracted_data", None, False)
        print ("ACTION OUTPUT : Downloaded extracted data from management subsystem and saved in directory, ", DATA_DIR + "/" + mgmtSubsystemName + "/extracted_data")
        time.sleep(20)
        runKubernetesCommand("delete -f " + extractDownloadDeploy, args.mgmt_ns)
    else:
        print ("ACTION OUTPUT : Not downloading extracted csv to the machine where script is run. The csv has to be explicitely downloaded from the pvc, ", pvc_name)
    ...
    if mgmtSubsystemName != None and config["management_subsystem"][mgmtSubsystemName]:
        ...
        for org in orgs["results"]:
            ...
            for azone in azones["results"]:
                ...
                analytics = runCommand("apic analytics-services:list --fields id,client_endpoint,name,title,availability_zone_url --server " + cloudAdminServer + " --org " + org["name"] + " --availability-zone " + azone["name"], True)
                analytics = yaml.safe_load(analytics)
                for eachAnalytics in analytics["results"]:
                    config["management_subsystem"][mgmtSubsystemName]["registered_analytics"][eachAnalytics["client_endpoint"]] = {
                    ...
                    gateway_portal_mapping["analytics_mapping"][eachAnalytics["name"]] = {
                        eachAnalytics["client_endpoint"]: "https://NEW_ANALYTICS_CLIENT_ENDPOINT_HOST_IN_TARGET_SYSTEM"
                    }
        ...
        with open(gateway_portal_mapping_filename, 'w') as mFile:
            ...
            mFile.write("#ac-endpoint url is the analytics client url or https://NEW_ANALYTICS_CLIENT_ENDPOINT_HOST_IN_TARGET_SYSTEM on the right side\n")
            mFile.write("\n")
