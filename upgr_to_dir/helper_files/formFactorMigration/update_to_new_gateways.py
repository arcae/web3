...
def createMigrationUserData():
    ...
    user = runCommand("apic users:get --org admin --server " + cloudAdminServer + " --user-registry " + migration_ur_name + " " + migration_user_name + " --output " + DATA_TEMP_DIR, True, 1, False)
    if user.startswith("Error") == True:
        userCreateRequest = {
        ...
        print ("ACTION OUTPUT : Migration user already exists. Details : ", user)
    ...
    if selectedSubsystem == None:
        ...
        for key in gatewaysFromSouceSystem:
            ...
            if "NEW_ENDPOINT" in eachGW and "NEW_URL" in eachGW:
                print("NEW GATEWAY INSTANCE IS CREATED USING SCRIPT. New gateway with NAME : {} , ENDPOINT : {}, API_ENDPOINT_BASE : {}".format(eachGW["NEW_NAME"], eachGW["NEW_ENDPOINT"], eachGW["NEW_API_ENDPOINT_BASE"])) 
                ...
                print ("NEW GATEWAY INSTANCE IS 'NOT' CREATED USING THE SCRIPT for this source system gateway.")
        print()
        print()
            ...
            for eachCatalog in catalogs["results"]:
                ...
                if spacesEanbled:
                    ...
                    for eachSpace in spaces["results"]:
                        ...
                        if c1 > 0:
                            ...
                            for each in old2newConfGWURLMAP:
                                ...
                                a3 = runCommand("apic configured-gateway-services:delete --scope space --org " + eachPORG["name"] + " --catalog "+ eachCatalog["name"] + " --space "+ eachSpace["name"] + " --server " + providerOrgServer + " " + confGWId, True) 
                    ...
                    if c1 > 0:
                        ...
                        for each in old2newConfGWURLMAP:
                            confGWId = each[each.rindex("/")+1:]
                            a3 = runCommand("apic configured-gateway-services:delete --scope catalog --org " + eachPORG["name"] + " --catalog "+ eachCatalog["name"] + " --server " + providerOrgServer + " " + confGWId, True) 
        ...
        if "registered_analytics" in managementSubsystemDetails and len(managementSubsystemDetails["registered_analytics"]) > 0:
            ...
            for eachAnalyticsClientHost in analyticsFromSouceSystem:
                eachAnalytics = analyticsFromSouceSystem[eachAnalyticsClientHost]
                if eachAnalyticsClientHost in aServiceMap: # source analytics record still existing in db
                    azonename = aServiceMap[eachAnalyticsClientHost]
                    ...
                    if gateway_portal_mapping != None and "analytics_mapping" in gateway_portal_mapping and eachAnalytics["name"] in gateway_portal_mapping["analytics_mapping"]:
                        clientEndpointMap = gateway_portal_mapping["analytics_mapping"][eachAnalytics["name"]]
                        if eachAnalyticsClientHost in clientEndpointMap:
                            newACEndpoint = clientEndpointMap[eachAnalyticsClientHost]
                            if newACEndpoint != None:
                                newACEndpoint = newACEndpoint.strip()   
                                if newACEndpoint != "" and newACEndpoint != "https://NEW_ANALYTICS_CLIENT_ENDPOINT_HOST_IN_TARGET_SYSTEM":
                                    if newACEndpoint.startswith("https://") == False:
                                        newACEndpoint = "https://" + newACEndpoint

                                    # check using endpoint because configurator job can insert analytics service with a specific name even though it exists
                                    if newACEndpoint not in aServiceMap:#checking if new service already created check
                                        apayload = {
                                            "name": eachAnalytics["name"]+"new",
                                            "title": eachAnalytics["name"]+"new",
                                            "endpoint": newACEndpoint
                                        }
                                        ...
                                        aname = aServiceMap[newACEndpoint]
...
def getAnalyticsServices():
    ...
    for org in orgs["results"]:
        ...
        azones = yaml.safe_load(azones)
        for azone in azones["results"]:
            services = runCommand("apic analytics-services:list --fields id,endpoint,name --server " + cloudAdminServer + " --org " + org["name"] + " --availability-zone " + azone["name"], True)
            services = yaml.safe_load(services)
            for service in services["results"]:
                output[service["endpoint"]] =  service["name"] + "^" + azone["name"]
    print ()
...
def start():
    ...
    print()
    print("Next steps:: Run post migration health check")
    print("********************************************")
    print("1. Run the health_check_post_migration.py script.")
